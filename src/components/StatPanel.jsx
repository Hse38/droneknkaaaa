    ...
    <div style={{background:'rgba(12,16,28,0.65)',backdropFilter:'blur(12px)',borderLeft:'1px solid var(--border)',display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'12px 14px',display:'flex',flexDirection:'column',gap:8}}>
            ...
            <div key={k}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:3}}>
                <div style={{display:'flex',alignItems:'center',gap:5}}>
                  <span style={{fontSize:12}}>{STAT_ICONS[k]}</span>
                  <span style={{fontFamily:'var(--mono)',fontSize:11,color:'var(--text2)',letterSpacing:0.5,textTransform:'uppercase'}}>{STAT_DISPLAY[k]}</span>
                </div>
                <span style={{fontFamily:'var(--mono)',fontSize:13,fontWeight:700,color}}>{val} <span style={{color:'var(--text3)',fontWeight:400,fontSize:10}}>/100</span></span>
              </div>
        ...
        <div style={{padding:'8px 10px',borderRadius:6,background:`${compatStatus.color}0f`,border:`1px solid ${compatStatus.color}33`,marginTop:4}}>
          <div style={{fontFamily:'var(--mono)',fontSize:10,letterSpacing:2,color:'var(--text3)',marginBottom:4,textTransform:'uppercase'}}>SİSTEM UYUMLULUĞU</div>
          <div style={{display:'flex',alignItems:'flex-start',gap:6}}>
            <span style={{color:compatStatus.color,fontSize:12,flexShrink:0}}>{compatStatus.ok?'✓':'⚠'}</span>
            <span style={{fontSize:12,color:compatStatus.color,lineHeight:1.5}}>{compatStatus.label}</span>
          </div>
        </div>
        ...
        <div style={{display:'flex',justifyContent:'center',padding:'4px 0'}}>
          <div style={{textAlign:'center'}}>
            <div style={{fontFamily:'var(--mono)',fontSize:10,letterSpacing:2,color:'var(--text3)',marginBottom:6,textTransform:'uppercase'}}>PERFORMANS ÖZETİ</div>
        ...
        <div style={{background:'var(--bg3)',borderRadius:6,padding:'10px 12px',border:'1px solid var(--border)'}}>
          <div style={{fontFamily:'var(--mono)',fontSize:10,letterSpacing:2,color:'var(--text3)',marginBottom:8,textTransform:'uppercase'}}>TAHMİNİ DEĞERLER</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            {[
              ...
              <div key={item.label}>
                <div style={{fontSize:11,color:'var(--text3)',marginBottom:2}}>{item.icon} {item.label}</div>
