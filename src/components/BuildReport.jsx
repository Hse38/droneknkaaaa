    ...
    <div style={{ height:'100vh', overflow:'auto', background: mission?.bgGradient || 'linear-gradient(135deg,#060810,#0f172a)', padding:24 }}>
      <div style={{ maxWidth:1180, margin:'0 auto', display:'flex', flexDirection:'column', gap:16 }}>
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid var(--border)', borderRadius:12, padding:14 }}>
          <div style={{ fontFamily:'var(--mono)', fontSize:10, letterSpacing:2, marginBottom:10 }}>GENEL PERFORMANS ÖZETİ</div>
          {STAT_KEYS.map((k, i) => (
        ...
        <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1.1fr 0.9fr', gap:14 }}>
          <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid var(--border)', borderRadius:12, padding:14 }}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {archetype.secondary.map((s) => (
                <span key={s.name} style={{ padding:'5px 9px', borderRadius:999, border:'1px solid var(--border2)', background:'rgba(255,255,255,0.03)', fontSize:12 }}>
          ...
          <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid var(--border)', borderRadius:12, padding:14 }}>
            <div style={{ fontFamily:'var(--mono)', fontSize:10, letterSpacing:2, marginBottom:10 }}>ARCHETYPE MATCH SCORES</div>
            {sortedArchetypes.map(([name, score], idx) => (
              <div key={name} style={{ marginBottom:8 }}>
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:12 }}><span>{name}</span><span>%{score}</span></div>
                <div style={{ height:6, borderRadius:3, background:'var(--bg4)' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:14 }}>
          ...
          <div style={{ background:'rgba(120,30,30,0.15)', border:'1px solid rgba(239,68,68,0.35)', borderRadius:12, padding:14 }}>
            <div style={{ fontFamily:'var(--mono)', fontSize:10, marginBottom:8 }}>ZAYIF YÖNLER</div>
            {weaknesses.map((k) => <div key={k} style={{ fontSize:13, marginBottom:5 }}>✗ {STAT_DISPLAY[k]} ({stats[k]})</div>)}
          </div>
        </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':isTablet?'repeat(2,1fr)':'repeat(3, 1fr)', gap:10 }}>
            {sims.map((s) => (
              <div key={s.id} style={{ border:'1px solid var(--border2)', borderRadius:10, padding:10, background:'rgba(0,0,0,0.15)' }}>
                ...
                <div style={{ fontSize:12, color:'var(--text2)' }}>{s.type} • {s.size}</div>
                <div style={{ marginTop:4, fontSize:12 }}>{s.desc}</div>
                <div style={{ marginTop:8, display:'inline-block', padding:'2px 8px', borderRadius:999, background:'rgba(0,212,255,0.12)', border:'1px solid rgba(0,212,255,0.4)', fontSize:11 }}>
        ...
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid var(--border)', borderRadius:12, padding:14 }}>
          <div style={{ fontFamily:'var(--mono)', fontSize:10, letterSpacing:2, marginBottom:8 }}>📚 ÖĞRENME NOTLARI</div>
